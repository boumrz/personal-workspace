...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await pool.query(
      "SELECT id, login, email, name, last_name, first_name, middle_name, age FROM users WHERE id = $1",
      [userId]
    );
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const query = `
      ...
      SET ${updates.join(", ")}
      WHERE id = $${paramCount}
      RETURNING id, login, email, name, last_name, first_name, middle_name, age
    `;
