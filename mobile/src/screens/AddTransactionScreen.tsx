...
export default function AddTransactionScreen({ navigation }: any) {
  ...
  const availableCategories = type === "income" ? categories.filter((c) => c.name === "Зарплата" || c.name === "Другое") : categories;
    ...
    (async () => {
      try {
        ...
        if (data.length > 0 && !selectedCategory) {
          const def = type === "income" ? data.find((c) => c.name === "Зарплата") || data[0] : data[0];
  ...
  const onSave = async () => {
    ...
    setSaving(true);
    try {
      await api.createTransaction({ type, amount: num, category: selectedCategory, description: description.trim() || "", date: formatDateForInput(date) });
      navigation.goBack();
    } catch (e: any) { Alert.alert("Ошибка", e?.message ?? "Не удалось добавить операцию"); }
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    typeBtn: { flex: 1, paddingVertical: 12, alignItems: "center", backgroundColor: theme.bgSurface, borderRadius: theme.radiusLg, paddingHorizontal: 20 },
    typeBtnActive: { backgroundColor: theme.bgCard, shadowColor: theme.shadowMd, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 1, shadowRadius: 8, elevation: 2 },
    ...
    categoryChip: { paddingVertical: 8, paddingHorizontal: 14, backgroundColor: theme.bgCard, borderRadius: 20, borderWidth: 1, borderColor: theme.border },
    categoryChipActive: { backgroundColor: theme.accentMuted, borderColor: theme.accentMuted },
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.categories}>
        {availableCategories.map((c) => (
          <TouchableOpacity key={c.id} style={[styles.categoryChip, selectedCategory?.id === c.id && styles.categoryChipActive]} onPress={() => setSelectedCategory(c)}>
            <Text style={[styles.categoryChipText, selectedCategory?.id === c.id && styles.categoryChipTextActive]} numberOfLines={1}>{c.name}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <TextInput style={styles.input} value={description} onChangeText={setDescription} placeholder="Описание" placeholderTextColor={theme.textTertiary} />
