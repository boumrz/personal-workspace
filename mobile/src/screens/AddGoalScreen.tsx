...
export default function AddGoalScreen({ navigation, route }: any) {
  ...
  const onSave = async () => {
    ...
    if (isNaN(target) || target <= 0) {
      Alert.alert("Ошибка", "Введите корректную целевую сумму");
      return;
    }
    ...
    try {
      ...
      Alert.alert("Ошибка", e?.message ?? "Не удалось сохранить цель");
    } finally {
      setSaving(false);
    }
  };
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      >
        <Text style={styles.saveBtnText}>
          {saving ? "Сохранение…" : editingGoal ? "Сохранить" : "Создать"}
        </Text>
      </TouchableOpacity>
    </ScrollView>
  );
